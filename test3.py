...
def _apply_kind_template(layer: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(template, dict):
        return layer
    return {key: _fill_value_from_template(template_value, layer.get(key)) for key, template_value in template.items()}
...
def normalize_export_layer_to_template(layer: Dict[str, Any]) -> Dict[str, Any]:
    kind = layer.get("kind")
    if kind in IMAGE_LIKE_EXPORT_KINDS or (kind == "ppt_graph_geo" and _kind_template_has_key(kind, "saved_path")):
...
def _svg_bytes_to_png_bytes(svg_bytes: bytes, out_w: int, out_h: int) -> Optional[bytes]:
    return _svg_bytes_to_png_via_rsvg_convert(svg_bytes, out_w, out_h)
...
def extract_graph_layer(
    ...
    if shape_spec is None:
        ...
        if _is_outer_page_border_graph_layer(out):
            return None
        if kind == "ppt_graph_geo" and _kind_template_has_key(kind, "saved_path"):
...
def extract_slide_canvas_layer(
    ...
    out_w = max(1, int(round((float(slide_width_emu) / EMU_PER_PT) * (96.0 / 72.0))))
    out_h = max(1, int(round((float(slide_height_emu) / EMU_PER_PT) * (96.0 / 72.0))))

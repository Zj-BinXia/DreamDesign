...
# #region debug-point H3:line-visual-mismatch-combine
...
def _normalize_layers_payload(payload: Any) -> List[Dict[str, Any]]:
    ...
    for layer in payload:
        if isinstance(layer, dict):
            flat_layers.append(_collapse_null_skeletons(dict(layer)))
    return flat_layers
...
def _extract_slide_background_fill(slide_layers: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    for layer in slide_layers:
        ...
        if len(bg_rgb) == 6:
            return {"type": "solid", "color_type": "RGB", "rgb": bg_rgb.upper()}
    return None
...
def _apply_slide_background_fill(slide, background_fill: Optional[Dict[str, Any]], background_spec: Optional[Dict[str, Any]] = None) -> None:
    ...
    color_spec = _normalized_color_spec(background_fill)
    if isinstance(color_spec, dict) and color_spec.get("type") == "srgb" and color_spec.get("rgb"):
        try:
            fill.fore_color.rgb = RGBColor.from_string(str(color_spec["rgb"]))
        except Exception:
            pass
    elif isinstance(color_spec, dict) and color_spec.get("type") == "scheme" and color_spec.get("scheme"):
        try:
            ...
            if sppr is not None:
                for child in list(sppr):
                    if child.tag == qn("a:solidFill"):
                        sppr.remove(child)
                solid = _color_fill_element(color_spec)
...
def _iter_color_mods(color_spec: Any) -> List[Dict[str, Any]]:
    if not isinstance(color_spec, dict):
        return []
    mods: List[Dict[str, Any]] = []
    alpha_val = color_spec.get("alpha_val")
    if alpha_val is not None:
        try:
            mods.append({"op": "alpha", "val": int(alpha_val)})
        except Exception:
            pass
    for mod in color_spec.get("mods") or []:
        if not isinstance(mod, dict) or not mod.get("op") or mod.get("val") is None:
            continue
        mods.append({"op": str(mod.get("op")), "val": mod.get("val")})
    return mods


def _normalized_color_spec(color_spec: Any, *, default_rgb: Optional[str] = None) -> Optional[Dict[str, Any]]:
    if not isinstance(color_spec, dict):
        return None
    spec = dict(color_spec)
    kind = str(spec.get("type") or spec.get("color_type") or "").strip().lower()
    scheme = str(spec.get("scheme") or "").strip()
    rgb = str(spec.get("rgb") or "").strip().lstrip("#").upper()
    if scheme:
        spec["scheme"] = scheme
        spec["type"] = "scheme"
        spec.pop("rgb", None)
        ...
        if len(rgb) != 6:
            if default_rgb is None:
                return None
            rgb = str(default_rgb).strip().lstrip("#").upper()
        if len(rgb) != 6:
            return None
        spec["rgb"] = rgb
        if kind != "scheme":
            spec["type"] = "srgb"
        spec.pop("scheme", None)
    if spec.get("alpha_val") is None:
        spec["alpha_val"] = 100000
    return spec


def _color_fill_element(color_spec: Any, *, default_rgb: Optional[str] = None):
    spec = _normalized_color_spec(color_spec, default_rgb=default_rgb)
    if not isinstance(spec, dict):
        return None
    solid = OxmlElement("a:solidFill")
    kind = str(spec.get("type") or "").lower()
    if kind == "scheme" and spec.get("scheme"):
        clr = OxmlElement("a:schemeClr")
        clr.set("val", str(spec.get("scheme")))
    elif spec.get("rgb"):
        clr = OxmlElement("a:srgbClr")
        clr.set("val", str(spec.get("rgb")).lstrip("#"))
    else:
        return None
    for mod in _iter_color_mods(spec):
        if not isinstance(mod, dict) or not mod.get("op") or mod.get("val") is None:
            continue
...
def _apply_spacing_node(
    ...
    mode = spacing.get("mode") or "points"
    raw = spacing.get("raw")
    rel = spacing.get("rel")
    ...
    if isinstance(end_para, dict) and isinstance(end_para.get("color"), dict):
        try:
            ...
            for child in list(end_el):
                if child.tag == qn("a:solidFill"):
                    end_el.remove(child)
            solid = _color_fill_element(end_para.get("color"), default_rgb="000000")
            if solid is not None:
                end_el.append(solid)
        except Exception:
            pass
...
def _set_run_font(run, run_info: Dict[str, Any], slide_size_pt: Optional[Tuple[float, float]] = None) -> None:
    ...
    strike_val = run_info.get("strike")
    if strike_val is None and isinstance(raw_rpr, dict):
        strike_val = raw_rpr.get("strike")
    def _apply_strike_override(rpr_el) -> None:
        if rpr_el is None:
            return
        if strike_val is True:
            rpr_el.set("strike", "sngStrike")
        elif strike_val and str(strike_val) != "noStrike":
            rpr_el.set("strike", str(strike_val))
        elif rpr_el.get("strike") is not None:
            del rpr_el.attrib["strike"]
    def _insert_solid_fill_before_fonts(rpr_el, solid_el) -> None:
        if rpr_el is None or solid_el is None:
            return
        font_like_tags = {
            qn("a:rFonts"),
            qn("a:latin"),
        ...
        for idx, child in enumerate(list(rpr_el)):
            if child.tag in font_like_tags:
                rpr_el.insert(idx, solid_el)
                return
        rpr_el.append(solid_el)
    ...
    if isinstance(raw_rpr, dict) and raw_rpr.get("from_fields"):
        try:
            ...
            if color:
                try:
                    solid = _color_fill_element(color, default_rgb="000000")
                    if solid is not None:
                        _insert_solid_fill_before_fonts(rpr, solid)
                except Exception:
                    pass
    ...
    if isinstance(raw_rpr, str) and raw_rpr.strip():
        try:
            ...
            _apply_strike_override(new_rpr)
            old_rpr = run._r.find(qn("a:rPr"))
            if old_rpr is not None:
                run._r.remove(old_rpr)
    ...
    if color:
        try:
            ...
            solid = _color_fill_element(color, default_rgb="000000")
            if solid is not None:
                _insert_solid_fill_before_fonts(r_pr, solid)
        except Exception:
            pass
    try:
        r_pr = run._r.get_or_add_rPr()
        _apply_strike_override(r_pr)
        if run_info.get("char_spacing_raw") is not None:
...
def _add_text_layer(slide, layer: Dict[str, Any], shift_x: float, shift_y: float) -> None:
    ...
    # Apply bodyPr using exported anchor plus built-in defaults, while still
    # accepting legacy full body_pr specs.
    try:
        bp = _text_body_pr_spec(layer)
    ...
    try:
        ...
        def _apply_mods(color_el, mods) -> None:
            for mod in _iter_color_mods(mods):
                if not isinstance(mod, dict) or not mod.get("op") or mod.get("val") is None:
                    continue
                m = OxmlElement(f"a:{str(mod['op'])}")
                m.set("val", str(mod.get("val")))
...
def _text_body_pr_spec(layer: Dict[str, Any]) -> Dict[str, Any]:
    attrs: Dict[str, Any] = {
        "anchor": layer.get("body_pr_anchor") or "t",
        "rtlCol": "0",
        "tIns": 0,
        "lIns": 0,
        "bIns": 0,
    ...
    if isinstance(body_pr, dict):
        old_attrs = body_pr.get("attrs")
        if isinstance(old_attrs, dict):
            for key, value in old_attrs.items():
                if value is None or str(key).endswith("_rel"):
                    continue
                attrs[str(key)] = value
        if body_pr.get("autofit") in {"spAutoFit", "normAutoFit", "noAutoFit"}:
            autofit = str(body_pr.get("autofit"))
    return {"attrs": attrs, "autofit": autofit}
...
def _add_graph_layer(slide, layer: Dict[str, Any], shift_x: float, shift_y: float) -> None:
    ...
    if isinstance(shape_xml, dict) and shape_xml.get("tag"):
        ...
        fmt = shape_xml.get("format") or ("shape_spec_v1" if layer.get("kind") == "ppt_graph_line" else None)
        stype = shape_xml.get("type") or ("line" if layer.get("kind") == "ppt_graph_line" else None)
        ...
        def _apply_color_spec(parent_el, color_spec: Any) -> None:
            ...
            if color_spec.get("type") == "srgb" and color_spec.get("rgb"):
                ...
                return
            for mod in _iter_color_mods(color_spec):
        ...
        def _clear_shape_theme_style(shp) -> None:
            try:

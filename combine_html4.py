...
def _collapse_null_skeletons(value: Any) -> Any:
    if isinstance(value, dict):
        collapsed = {k: _collapse_null_skeletons(v) for k, v in value.items()}
        if not collapsed:
            return None
        if all(v is None for v in collapsed.values()):
            return None
        return collapsed
    if isinstance(value, list):
        collapsed_items = [_collapse_null_skeletons(v) for v in value]
        collapsed_items = [item for item in collapsed_items if item is not None]
        return collapsed_items or None
    return value
...
def _normalize_layers_payload(payload: Any) -> Tuple[Any, List[Dict[str, Any]]]:
    ...
    if raw and all(isinstance(item, list) for item in raw):
        ...
        for slide_idx, slide_layers in enumerate(raw, start=1):
            ...
            for layer_idx, layer in enumerate(slide_layers):
                ...
                normalized = _collapse_null_skeletons(normalized)
    ...
    slide_layer_counts: Dict[int, int] = {}
    global_index = 0
    for layer in raw:
        if isinstance(layer, dict):
            normalized = _collapse_null_skeletons(dict(layer))
            try:
                slide_no = int(normalized.get("slide") or 1)
            except Exception:
                slide_no = 1
            normalized.setdefault("slide", slide_no)
            normalized.setdefault("layer_index_in_slide", slide_layer_counts.get(slide_no, 0))
            normalized.setdefault("layer_index_global", global_index)
            slide_layer_counts[slide_no] = int(normalized.get("layer_index_in_slide", 0)) + 1
            global_index += 1
            flat_layers.append(normalized)
...
def _css_color_from_spec(spec: Optional[Dict[str, Any]], default: str = "transparent") -> str:
    ...
    alpha = 1.0
    alpha_val = spec.get("alpha_val")
    if alpha_val is not None:
        try:
            alpha = max(0.0, min(1.0, float(alpha_val) / 100000.0))
        except Exception:
            alpha = 1.0
...
def _decoration_enabled(value: Any, *, strike: bool = False) -> bool:
    if value is True:
        return True
    ...
    if not isinstance(value, str):
        return bool(value)
    token = value.strip()
    if token in {"", "none", "false", "noStrike"}:
        return False
    if strike:
        return "strike" in token.lower()
    return True


def _text_decoration_css(run: Dict[str, Any]) -> Optional[str]:
    parts: List[str] = []
    if _decoration_enabled(run.get("underline")):
        parts.append("underline")
    if _decoration_enabled(run.get("strike"), strike=True):
...
def _layer_anchor(layer: Dict[str, Any]) -> str:
    if layer.get("body_pr_anchor") is not None:
        return str(layer.get("body_pr_anchor") or "t").lower()
...
def _text_body_pr_autofit(layer: Dict[str, Any]) -> str:
    body_pr = layer.get("body_pr")
    if isinstance(body_pr, dict) and body_pr.get("autofit") in {"spAutoFit", "normAutoFit", "noAutoFit"}:
        return str(body_pr.get("autofit"))
    return "spAutoFit"
...
def _paragraph_alignment_css(para: Dict[str, Any]) -> str:
    direct = para.get("alignment")
    if direct is not None:
        return _alignment_css(direct)
    attrs = para.get("ppr_attrs")
    if isinstance(attrs, dict):
        token = str(attrs.get("algn") or "").lower()
        mapping = {
            "l": "left",
            "ctr": "center",
            "r": "right",
...
def _text_autofit_scale(layer: Dict[str, Any], canvas_size_emu: Tuple[int, int]) -> float:
    if _text_body_pr_autofit(layer) != "spAutoFit":
        return 1.0
    ...
    for para in layer.get("paragraphs") or []:
        line_pt = None
        if isinstance(para.get("line_spacing"), dict) and str(para["line_spacing"].get("mode") or "points") == "points":
            try:
...
def _svg_text_align(para: Dict[str, Any], left: float, right: float) -> Tuple[str, float]:
    align = _paragraph_alignment_css(para)
    if align == "center":
...
def _svg_run_style(run: Dict[str, Any], font_size_pt: float, letter_spacing_pt: Optional[float]) -> str:
    ...
    decoration = _text_decoration_css(run)
    if decoration:
        styles.append(f"text-decoration:{decoration}")
    return ";".join(styles)


def _html_run_style(run: Dict[str, Any], font_size_pt: float, letter_spacing_pt: Optional[float]) -> str:
    styles = []
    if run.get("font_name"):
        styles.append(f"font-family:{_css_font_family(run['font_name'])}")
    styles.append(f"font-size:{font_size_pt:.2f}pt")
    styles.append(f"font-weight:{_font_weight(run.get('bold'))}")
    styles.append(f"font-style:{_font_style(run.get('italic'))}")
    styles.append(f"color:{_css_color_from_spec(run.get('color'), '#000000')}")
    styles.append("font-kerning:none")
    styles.append("font-variant-ligatures:none")
    styles.append("font-feature-settings:'kern' 0, 'liga' 0, 'clig' 0")
    if letter_spacing_pt is not None:
        styles.append(f"letter-spacing:{letter_spacing_pt:.2f}pt")
    decoration = _text_decoration_css(run)
    if decoration:
        styles.append(f"text-decoration:{decoration}")
    return ";".join(styles)
...
def _can_render_text_layer_as_plain_html(
    layer: Dict[str, Any],
    ...
    try:
        ...
        rot = 0.0
    abs_rot = abs(rot) % 360.0
    if abs_rot > 180.0:
        abs_rot = 360.0 - abs_rot
    # Keep plain HTML as a narrow fallback for large-angle rotated labels/titles.
    # Most text matches PowerPoint more closely through the SVG path, including
    # ordinary single-line titles/dates/addresses and slightly rotated display text.
    is_large_angle_rotation = 45.0 <= abs_rot <= 135.0
    if not para_metrics or not is_large_angle_rotation:
        return False
    for item in para_metrics:
        para = item[0]
        wrapped_lines = item[4]
        if _paragraph_bullet_char(para):
            return False
        para_runs = [run for run in (para.get("runs") or []) if isinstance(run, dict) and str(run.get("text") or "")]
        is_single_run_no_break = (
            len(para_runs) == 1
            and "\n" not in str(para_runs[0].get("text") or "")
        )
        if len(wrapped_lines) != 1 and not is_single_run_no_break:
            return False
    return True


def _render_plain_html_text_layer(
    ...
    outer_bits = list(container_bits)
    if fill != "transparent":
        outer_bits.append(f"background:{fill}")
    if border_color != "transparent" and border_w > 0:
        outer_bits.append(f"border:{border_w:.2f}pt solid {border_color}")
    outer_bits.extend(
        [
            "display:flex",
            "flex-direction:column",
            f"justify-content:{_vertical_align_css(anchor)}",
            "align-items:stretch",
    ...
    for para, before_pt, line_height_pt, after_pt, wrapped_lines in para_metrics:
        ...
        if is_single_run_no_break:
            font_size_pt, letter_spacing_pt, font_path = _run_font_metrics(para_runs[0], autofit_scale, doc_dir, canvas_size_emu)
            _ = font_path
            line_runs = [
                {
                    "run": para_runs[0],
                    "text": str(para_runs[0].get("text") or ""),
            ...
            line_runs = _merge_line_runs(wrapped_lines[0] if wrapped_lines else [])
        spans: List[str] = []
        for item in line_runs:
            ...
            if not text:
                continue
            spans.append(
                f'<span style="{_html_run_style(item["run"], float(item["font_size_pt"]), item["letter_spacing_pt"])}">{text}</span>'
            )
        para_style = _style_string(
            [
                "margin:0",
                f"text-align:{_paragraph_alignment_css(para)}",
                "white-space:pre",
                f"line-height:{line_height_pt:.2f}pt" if line_height_pt > 0 else "",
                f"min-height:{line_height_pt:.2f}pt" if line_height_pt > 0 else "",
                f"margin-top:{before_pt:.2f}pt" if before_pt > 0 else "",
                f"margin-bottom:{after_pt:.2f}pt" if after_pt > 0 else "",
                f"margin-left:{_paragraph_left_margin_pt(para, canvas_size_emu):.2f}pt",
                f"text-indent:{_paragraph_indent_pt(para, canvas_size_emu):.2f}pt",
            ]
        )
        para_nodes.append(f'  <div style="{para_style}">{"".join(spans)}</div>')

    parts = [f'<div class="layer text-layer text-layer-html" data-kind="text" style="{_style_string(outer_bits)}">']
    parts.extend(para_nodes)
    parts.append("</div>")
    return "\n".join(parts)
...
def _run_font_metrics(
    ...
    # Browsers collapse decorative display fonts much more aggressively than
    # PowerPoint when negative letter-spacing is applied. Keep positive tracking,
    # but drop negative tracking in HTML preview.
    if base_spacing_pt is not None and base_spacing_pt >= 0:
...
def _wrap_paragraph_runs(
    ...
    # Decorative Latin headline layers like "THANKS" are often a single run with
    # a single token. Keep them on one line to avoid false per-character splits.
    # CJK headlines commonly omit spaces, so they must still be allowed to wrap.
    para_runs = [run for run in (para.get("runs") or []) if isinstance(run, dict) and str(run.get("text") or "")]
    if len(para_runs) == 1:
        only_run = para_runs[0]
        only_text = str(only_run.get("text") or "")
        if (
            only_text
            and ("\n" not in only_text)
            and (not any(ch.isspace() for ch in only_text))
            and (not _text_contains_cjk(only_text))
            and (len(only_text) <= 6)
        ):
            font_size_pt, letter_spacing_pt, font_path = _run_font_metrics(only_run, autofit_scale, doc_dir, canvas_size_emu)
            return [[{
                "run": only_run,
                "text": only_text,
                "font_size_pt": font_size_pt,
    ...
    # Fast path: if the paragraph has no explicit line break and the full measured
    # width already fits, keep it on a single line. This avoids token-by-token wrap
    # overestimation for decorative fonts with negative letter-spacing.
    if para_runs and all("\n" not in str(run.get("text") or "") for run in para_runs):
        single_line: List[Dict[str, Any]] = []
        total_width = 0.0
        for run in para_runs:
            font_size_pt, letter_spacing_pt, font_path = _run_font_metrics(run, autofit_scale, doc_dir, canvas_size_emu)
            text = str(run.get("text") or "")
            total_width += _measure_text_width(text, font_size_pt, letter_spacing_pt, font_path)
            single_line.append(
                {
                    "run": run,
                    "text": text,
    ...
    def push_fragment(
        ...
        text: str,
        font_size_pt: float,
        letter_spacing_pt: Optional[float],
        font_path: Optional[Path],
    ) -> None:
        nonlocal current_width
        frag_width = _measure_text_width(text, font_size_pt, letter_spacing_pt, font_path)
        current_line.append(
            {
                "run": run,
                "text": text,
    ...
    def push_breakable_token(
        ...
        if token_width <= max_width_pt:
            if current_line and current_width + token_width > max_width_pt:
                flush_line()
            push_fragment(run, token, font_size_pt, letter_spacing_pt, font_path)
            return

        fragment = ""
        for ch in token:
            trial = fragment + ch
            trial_width = _measure_text_width(trial, font_size_pt, letter_spacing_pt, font_path)
            available = max_width_pt - current_width
            if current_line and trial_width > max(0.0, available):
                if fragment:
                    push_fragment(run, fragment, font_size_pt, letter_spacing_pt, font_path)
                    flush_line()
                    fragment = ch
                    continue
                flush_line()
            if (not current_line) and trial_width > max_width_pt and fragment:
                push_fragment(run, fragment, font_size_pt, letter_spacing_pt, font_path)
                flush_line()
                fragment = ch
                continue
            fragment = trial
        if fragment:
            frag_width = _measure_text_width(fragment, font_size_pt, letter_spacing_pt, font_path)
            if current_line and current_width + frag_width > max_width_pt:
                flush_line()
            push_fragment(run, fragment, font_size_pt, letter_spacing_pt, font_path)
    ...
    for run in para.get("runs") or []:
        ...
        for token in _tokenize_run_text(raw_text):
            ...
            if not current_line:
                if is_space:
                    continue
                push_breakable_token(run, token, font_size_pt, letter_spacing_pt, font_path)
                continue
...
def _render_text_layer(layer: Dict[str, Any], canvas_size_emu: Tuple[int, int], doc_dir: Path) -> str:
    ...
    if _can_render_text_layer_as_plain_html(layer, para_metrics):
        return _render_plain_html_text_layer(
            container_bits,
            fill,
            border_color,
            border_w,
...
def _render_line_layer(layer: Dict[str, Any]) -> str:
    ...
    if isinstance(color, dict) and "alpha_val" not in color:
        color = {**color, "alpha_val": 100000}
    stroke = _css_color_from_spec(color, "#000000")
    ...
    line_tag = (
        ...
        f'stroke-linecap="butt" stroke-linejoin="miter"{dash_attr}{marker_start}{marker_end}/>'
    )
    return "\n".join(
        [
            (
...
def _slide_background_css(slide_layers: List[Dict[str, Any]]) -> str:
    for layer in slide_layers:
        if layer.get("kind") != "slide_canvas":
            continue
        bg_rgb = str(layer.get("background_rgb") or "").strip().lstrip("#")
        if len(bg_rgb) == 6:
            return f"background:#{bg_rgb};"
...
def _render_slide(
    ...
    for layer in reversed(slide_layers):
        if layer.get("kind") == "slide_canvas":
            if layer.get("saved_path"):
                rendered = _render_image_layer(layer, html_out, assets_dir)
                if rendered:
